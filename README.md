...
## 왜 폴더 전체를 그대로 올리지 않나요?

Git에는 소스와 설정 파일만 올립니다.

아래 파일들은 컴퓨터마다 다르거나 자동 생성되므로 저장소에 올리지 않습니다.
...
다른 개발자가 clone해서 Android Studio로 열면 Gradle이 필요한 build 폴더를 다시 생성합니다. 그래서 저장소에는 현재처럼 소스만 올라가는 것이 정상입니다.
